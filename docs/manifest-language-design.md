...
A workspace has exactly one authoritative `zolt.lock`, at the workspace root. A member directory never has one, and no command ever reads or writes a member-local lockfile — including a command run from inside a member. This binds every lock consumer, not just the build: dependency queries (`zolt classpath`, `zolt classpath audit`, `zolt tree`, `zolt why`, `zolt conflicts`), supply-chain reports (`zolt licenses`, `zolt sbom`, publication SBOM attachment), policy checks, toolchain reads and `zolt toolchain sync`, `zolt exec`, package planning, and build provenance all resolve the same authoritative path. The one deliberate exception is the staleness gate that a build or resolve run inside a member uses to notice a member-local lock a standalone command left behind, whose sole purpose is to redirect the user to `--workspace --member`; it never treats that file as a source of resolved facts. Every plan, resolve, package, and test path therefore takes the authoritative lockfile path explicitly from the caller that already knows whether the directory is a member; nothing derives it by resolving `zolt.lock` against the project directory. Machine output names that path canonically and absolutely; human output may label it as the workspace lock, never as a `../..` relative path.
