...
A command started inside a member therefore has two roots, not one: its **project root**, which owns its manifest, sources, and outputs, and its **lock root**, which owns `zolt.lock`. Every command loads that pair once, at its boundary, as `ProjectCommandContext` — `(project, projectRoot, workspaceRoot, lockRoot, lockfilePath, memberPath)`. Standalone: `projectRoot == lockRoot` and `memberPath` is `.`. Member: `projectRoot` is `<workspace>/<memberPath>`, `lockRoot` is the workspace root. Nothing downstream re-derives either root, and nothing derives a lockfile path from a project directory.

**The core-request rule.** The command boundary decides the pair; the core operations must be *told* it. A build, test, or package request carries `ProjectBuildContext` — `(projectRoot, lockfilePath, memberPath)` — and every service downstream reads `lockfilePath` from the request rather than resolving one against `projectRoot`. Standalone entry points construct the context with `ProjectBuildContext.standalone(projectDirectory)`; the workspace hands each member `Workspace.memberContext(member)`, which pairs the member's directory with the workspace root's lockfile.

The rule binds extension points too. A framework adapter — `FrameworkBuildAugmenter`, `FrameworkPackageAugmenter`, `FrameworkRunAugmenter` — plans its own dependency closure in its own module, so an interface handing it only a project directory would leave it nothing to consult but the member's own directory. These seams therefore carry `ProjectBuildContext`, and an adapter that needs the lock reads the path it was given.

This is not a style preference, because a member-local `zolt.lock` is **observationally irrelevant**: planting, changing, or corrupting one must change nothing that any command computes, reports, caches, or fails on. A service that derives its own path defeats that even when it resolves the right dependencies, since the lockfile's content hash is a direct input to the main and test compile fingerprints, to the build-cache key on both scopes, and to tests-JAR evidence — so a stray file that nobody is allowed to read still forces recompiles, splits cache keys, and can fail packaging outright. The rule is enforced structurally: `ProjectLockfile.in(lockRoot)` may only be called where lock ownership is *established* — the command boundary, workspace root handling, the resolve writer, and standalone entry points — and the architecture guardrail `ProjectLockfileCallerGuardrailTest` holds the allowlist of those files, with the reason each one owns the decision.
...
A workspace has exactly one authoritative `zolt.lock`, at the workspace root. A member directory never has one, and no command ever reads or writes a member-local lockfile — including a command run from inside a member. This binds every lock consumer, not just the build: dependency queries (`zolt classpath`, `zolt classpath audit`, `zolt tree`, `zolt why`, `zolt conflicts`), supply-chain reports (`zolt licenses`, `zolt sbom`, publication SBOM attachment), policy checks, toolchain reads and `zolt toolchain sync`, `zolt exec`, package planning, and build provenance all resolve the same authoritative path. There is no exception. A member-local `zolt.lock` an older Zolt left behind is ignored outright: it is neither consulted, nor consumed, nor rewritten, and it is not a reason to refuse the command — a member-directory command routes through the workspace (§4.5) and reads the root lock, so no code path exists that could take that file for a source of resolved facts. Every plan, resolve, package, and test path takes the authoritative lockfile path explicitly from the caller that already knows whether the directory is a member; nothing derives it by resolving `zolt.lock` against the project directory, and an architecture guardrail keeps that derivation in one owner. Machine output names that path canonically and absolutely; human output may label it as the workspace lock, never as a `../..` relative path.
...
- `--no-resolve` means "do not refresh artifacts or `zolt.lock`". It never means "skip semantic validation". A `--no-resolve` edit still acquires the workspace mutation lock, recovers pending journals, patches the source in memory, composes the complete effective workspace with the edited member substituted, runs every workspace composition validation, rechecks the captured workspace inputs, and only then writes the manifest atomically, leaving `zolt.lock` untouched. It uses the same composition path as a resolving edit and merely stops before resolution, so a `--no-resolve` write can never redeclare a root-owned version, platform, or credential ID, introduce a `workspace = true` edge no member provides, collide with another member's effective project identity, or otherwise violate a workspace-only composition rule.
